...
<form class="shipping_calculator" action="<?php echo esc_url( $woocommerce->cart->get_cart_url() ); ?>" method="post">
	...
	<section class="shipping-calculator-form clearfix">
		...
		<p class="form-row form-row-last">
			<?php
				...
				if ( is_array( $states ) && empty( $states ) ) {
					...
					// Input
					?>
					<input type="text" class="input-ckr" value="<?php echo esc_attr( $current_r ); ?>" placeholder="<?php _e('State', 'woocommerce'); ?>" name="calc_shipping_state" id="calc_shipping_state" />
					<?php
		...
		<div class="clear"></div>
		<p class="form-row form-row-first">
			<input type="text" class="input-ckr" value="<?php echo esc_attr( $woocommerce->customer->get_shipping_postcode() ); ?>" placeholder="<?php _e('Postcode/Zip', 'woocommerce'); ?>" title="<?php _e('Postcode', 'woocommerce'); ?>" name="calc_shipping_postcode" id="calc_shipping_postcode" />
		</p>
		<p class="form-row form-row-last"><button type="submit" name="calc_shipping" value="1" class="button-ckr"><?php _e('Update Totals', 'woocommerce'); ?></button></p>
