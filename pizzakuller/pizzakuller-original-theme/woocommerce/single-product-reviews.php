...
<?php if ( comments_open() ) : ?><div id="reviews"><?php
	...
	if ( $count > 0 ) :
		...
		echo '<div class="reviews-header" itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating">';

		echo '<div class="rating" title="'.sprintf(__('Rated %s out of 5', 'woocommerce'), $average).'"><span style="width:'.($average*20).'%"><span itemprop="ratingValue" >'.$average.'</span> '.__('out of 5', 'woocommerce').'</span></div>';

		echo '<h2 class="heading">'.sprintf( _n('%s review for %s', '%s reviews for %s', $count, 'woocommerce'), '<span itemprop="ratingCount" class="count">'.$count.'</span>', wptexturize($post->post_title) ).'</h2>';

		echo '</div>';

	else :

		echo '<h2 class="heading">'.__('Reviews', 'woocommerce').'</h2>';
	...
	if ( have_comments() ) :
		...
		echo '<p class="add_review"><a href="#review_form" class="inline show_review_form button-ckr">'.__('Add Review', 'woocommerce').'</a></p>';
